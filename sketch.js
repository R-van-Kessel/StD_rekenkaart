...
function generateQuestions() {
    ...
    // ===== SOM 1: Verhoudingstabel (rijst/personen) =====
    {
        let basePersons = floor(random(2, 11));
        let gramsPerPerson = floor(random(3, 10)) * 10; // veelvoud van 10: 30-90g
        let baseGrams = basePersons * gramsPerPerson;
        let targetPersons = basePersons + floor(random(1, 10));
        let answer = Math.ceil(targetPersons * gramsPerPerson);
        questions.push({
            text: "Voor " + basePersons + " personen\nis er " + baseGrams + " gram rijst\nnodig. Hoeveel\nrijst is nodig\nvoor " + targetPersons + " personen?",
            answer: answer
        });
    }

    // ===== SOM 2: Verhoudingstabel (km/u naar m/s) =====
    {
        // Kies snelheden die netjes deelbaar zijn door 3.6
        let niceKmh = [18, 36, 54, 72, 90, 108, 126, 144];
        let kmh = random(niceKmh);
        let answer = Math.ceil(kmh / 3.6);
        questions.push({
            text: "Je hebt een\nsnelheid van\n" + kmh + " km/u\nHoeveel is\ndat in m/s?",
            answer: answer
        });
    ...
    // ===== SOM 3: Procenten – nieuwe prijs na korting =====
    {
        let kortingPct = random([10, 15, 20, 25, 30, 40, 50]);
        // Kies een basisprijs die een rond antwoord geeft
        let basePrice = floor(random(4, 20)) * 5; // 20-100 euro
        let newPrice = Math.ceil(basePrice * (1 - kortingPct / 100));
        questions.push({
            text:  "Je krijgt \n"+ kortingPct +" % korting.\nDe oude prijs was\n€" + basePrice + ". Wat is\nde nieuwe prijs?",
            answer: newPrice
        });
    ...
    // ===== SOM 4: Procenten – oude prijs terugrekenen =====
    {
        let kortingPct = random([10, 20, 25, 30, 40, 50]);
        // Maak een antwoord dat netjes deelbaar is
        let origPrice = floor(random(4, 20)) * 10; // 40-200 euro
        let newPrice = Math.ceil(origPrice * (1 - kortingPct / 100));
        questions.push({
            text: "Je krijgt \n"+ kortingPct +" % korting.\nDe nieuwe prijs is\n€" + newPrice + ". Wat was\nde oude prijs?",
            answer: origPrice
        });
    ...
    // ===== SOM 5: Formule – gemiddelde snelheid (m/s) =====
    {
        // Schaatser: v = afstand / tijd, kies waardes die netjes uitkomen
        let speed = floor(random(8, 20)); // m/s
        let time = floor(random(3, 10)) * 10; // seconden: 30-90
        let distance = speed * time;
        questions.push({
            text: "Een schaatser\n schaatst\n" + distance + "m in " + time + "s. \nWat is de snelheid\n in m/s?",
            answer: speed
        });
    }
// ===== SOM 6: Formule – taxirit (tijd terugrekenen) =====
{
    let tariefPerMin = random([0.25, 0.50, 0.75, 1.50, 2.00]);
    let starttarief = random([2, 3, 4, 5, 6, 8, 10]);

    let mogelijkeMinuten = [];
    for (let m = 5; m <= 40; m++) {
        let totaal = tariefPerMin * m + starttarief;
        if (abs(Math.round(totaal * 100) / 100 - Math.floor(totaal * 100) / 100) < 0.001) {
            mogelijkeMinuten.push(m);
        }
    }

    if (mogelijkeMinuten.length === 0) mogelijkeMinuten = [10, 20, 30];

    let minutes = random(mogelijkeMinuten);
    let bedrag = tariefPerMin * minutes + starttarief;
    bedrag = Math.ceil(bedrag * 100) / 100;

    let tariefTekst = (tariefPerMin % 1 === 0)
        ? tariefPerMin
        : nf(tariefPerMin, 1, 2).replace('.', ',');

    let bedragTekst = (bedrag % 1 === 0)
        ? bedrag
        : nf(bedrag, 1, 2).replace('.', ',');

    questions.push({
        text: "Taxirit kost:\nB = " + tariefTekst + "t + " + starttarief + "\nJe betaalt (B):\n € " + bedragTekst + "\nHoelang (t)\nwas de rit?",
        answer: minutes
    });
}

// ===== GEDEELD BASISCIJFER VOOR SOMMEN 7-10 =====
let basisGetal = floor(random(2, 10));

let conversions7 = [
    { from: basisGetal, unit: 'km', toUnit: 'cm', factor: 100000 },
    { from: basisGetal * 100, unit: 'cm', toUnit: 'm', factor: 1 / 100 },
    { from: basisGetal * 1000, unit: 'm', toUnit: 'km', factor: 1 / 1000 },
    { from: basisGetal, unit: 'm', toUnit: 'mm', factor: 1000 },
];

let conversions8 = [
    { from: basisGetal, unit: 'm²', toUnit: 'cm²', factor: 10000 },
    { from: basisGetal * 100000, unit: 'cm²', toUnit: 'm²', factor: 1 / 10000 },
    { from: basisGetal, unit: 'km²', toUnit: 'ha', factor: 100 },
    { from: basisGetal * 100, unit: 'ha', toUnit: 'km²', factor: 1 / 100 },
];

let conversions9 = [
    { from: basisGetal, unit: 'm³', toUnit: 'liter', factor: 1000 },
    { from: basisGetal * 1000, unit: 'liter', toUnit: 'm³', factor: 1 / 1000 },
    { from: basisGetal, unit: 'liter', toUnit: 'ml', factor: 1000 },
    { from: basisGetal * 1000, unit: 'ml', toUnit: 'liter', factor: 1 / 1000 },
];

let conversions10 = [
    { from: basisGetal, unit: 'ton', toUnit: 'kg', factor: 1000 },
    { from: basisGetal * 1000, unit: 'kg', toUnit: 'ton', factor: 1 / 1000 },
    { from: basisGetal, unit: 'kg', toUnit: 'g', factor: 1000 },
    { from: basisGetal * 1000, unit: 'g', toUnit: 'kg', factor: 1 / 1000 },
];

// Shuffle alle lijsten
conversions7 = shuffle(conversions7);
conversions8 = shuffle(conversions8);
conversions9 = shuffle(conversions9);
conversions10 = shuffle(conversions10);

// Kies combinatie waarbij alle 4 antwoorden uniek zijn
let c7, c8, c9, c10;
let found = false;

outer: for (let i = 0; i < conversions7.length; i++) {
    for (let j = 0; j < conversions8.length; j++) {
        for (let k = 0; k < conversions9.length; k++) {
            for (let l = 0; l < conversions10.length; l++) {
                let a7  = Math.ceil(conversions7[i].from  * conversions7[i].factor);
                let a8  = Math.ceil(conversions8[j].from  * conversions8[j].factor);
                let a9  = Math.ceil(conversions9[k].from  * conversions9[k].factor);
                let a10 = Math.ceil(conversions10[l].from * conversions10[l].factor);
                let antwoorden = [a7, a8, a9, a10];
                let uniek = new Set(antwoorden).size === 4;
                if (uniek) {
                    c7 = conversions7[i];
                    c8 = conversions8[j];
                    c9 = conversions9[k];
...
// Fallback als geen unieke combinatie gevonden (zou zelden voorkomen)
if (!found) {
    c7  = conversions7[0];
    c8  = conversions8[0];
    c9  = conversions9[0];
...
// ===== SOM 7: Lengtematen =====
questions.push({
    text: "Omrekenen:\n" + formatGetal(c7.from) + " " + c7.unit + "\n= hoeveel\n" + c7.toUnit + "?",
    answer: Math.ceil(c7.from * c7.factor)
});

// ===== SOM 8: Oppervlaktematen =====
questions.push({
    text: "Omrekenen:\n" + formatGetal(c8.from) + " " + c8.unit + "\n= hoeveel\n" + c8.toUnit + "?",
    answer: Math.ceil(c8.from * c8.factor)
});

// ===== SOM 9: Volume en inhoud =====
questions.push({
    text: "Omrekenen:\n" + formatGetal(c9.from) + " " + c9.unit + "\n= hoeveel\n" + c9.toUnit + "?",
    answer: Math.ceil(c9.from * c9.factor)
});

// ===== SOM 10: Massa en gewicht =====
questions.push({
    text: "Omrekenen:\n" + formatGetal(c10.from) + " " + c10.unit + "\n= hoeveel\n" + c10.toUnit + "?",
    answer: Math.ceil(c10.from * c10.factor)
});


    // ===== ANTWOORDEN OPHALEN EN SHUFFLEN =====
    ...
    for (let row = 3; row < 5; row++) {
        for (let col = 0; col < COLS; col++) {
            blocks.push({
                ...
                isDragging: false,
                isPlaced: true,
                text: formatGetal(answers[answerIndex]),
...
function drawBlock(block) {
  ...
  // Bepaal kleur
    ...
    textStyle(BOLD);
    textSize(14);
    textLeading(20);  // REGELAFSTAND (standaard is ongeveer 15-16)
