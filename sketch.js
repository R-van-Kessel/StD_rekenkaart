...
function generateQuestions() {
    ...
    // ===== SOM 1: Verhoudingstabel (rijst/personen) =====
    {
        ...
        let answer = targetPersons * gramsPerPerson;
        questions.push({
            text: "Voor " + basePersons + " personen\nheb je " + baseGrams + "g rijst\nnodig. Hoeveel\nvoor " + targetPersons + " personen?",
            answer: answer
        });
    }
    ...
    // ===== SOM 3: Procenten – nieuwe prijs na korting =====
    {
        ...
        let newPrice = round(basePrice * (1 - kortingPct / 100));
        questions.push({
            text: kortingPct + "% korting.\nOude prijs:\n€" + basePrice + ". Wat is\nde nieuwe prijs?",
            answer: newPrice
        });
    }
    ...
    // ===== SOM 4: Procenten – oude prijs terugrekenen =====
    {
        ...
        let newPrice = round(origPrice * (1 - kortingPct / 100));
        questions.push({
            text: kortingPct + "% korting.\nNieuwe prijs:\n€" + newPrice + ". Wat was\nde oude prijs?",
            answer: origPrice
        });
...
{
    // Random tarieven die nette antwoorden geven
    let tariefPerMin = random([0.25, 0.50, 0.75, 1.50, 2.00]);
    let starttarief   = random([2, 3, 4, 5, 6, 8, 10]);

    // Kies een even aantal minuten zodat tariefPerMin × minuten altijd heel uitkomt
    // Bepaal welke minuten-waarden een heel getal geven voor dit tarief
    let mogelijkeMinuten = [];
    for (let m = 5; m <= 40; m++) {
        let totaal = tariefPerMin * m + starttarief;
        if (abs(round(totaal) - totaal) < 0.001) { // controleer of uitkomst heel getal is
            mogelijkeMinuten.push(m);
        }
    }

    // Fallback als lijst leeg is (zou niet mogen gebeuren)
    ...
    // Formaat tarief netjes (geen onnodige decimalen)
    let tariefTekst = (tariefPerMin % 1 === 0) ? tariefPerMin : nf(tariefPerMin, 1, 2);
    ...
    // ===== SOM 7: Lengtematen omrekenen =====
    {
        let conversions = [
            { from: floor(random(1, 10)) * 100, unit: 'cm', toUnit: 'm', factor: 1 / 100 },
            { from: floor(random(1, 10)), unit: 'km', toUnit: 'm', factor: 1000 },
            { from: floor(random(1, 20)) * 10, unit: 'mm', toUnit: 'cm', factor: 1 / 10 },
            { from: floor(random(1, 5)), unit: 'km', toUnit: 'cm', factor: 100000 },
        ];
        let c = random(conversions);
        let answer = round(c.from * c.factor);
        questions.push({
            text: "Omrekenen:\n" + c.from + " " + c.unit + "\n= hoeveel\n" + c.toUnit + "?",
            answer: answer
        });
    ...
    // ===== SOM 8: Oppervlaktematen omrekenen =====
    {
        let conversions = [
            { from: floor(random(1, 10)), unit: 'm²', toUnit: 'cm²', factor: 10000 },
            { from: floor(random(1, 5)) * 10000, unit: 'cm²', toUnit: 'm²', factor: 1 / 10000 },
            { from: floor(random(1, 8)), unit: 'km²', toUnit: 'ha', factor: 100 },
            { from: floor(random(2, 10)) * 100, unit: 'ha', toUnit: 'km²', factor: 1 / 100 },
        ];
        let c = random(conversions);
        let answer = round(c.from * c.factor);
        questions.push({
    ...
    // ===== SOM 9: Volume en inhoud omrekenen =====
    {
        let conversions = [
            { from: floor(random(1, 10)), unit: 'liter', toUnit: 'ml', factor: 1000 },
            { from: floor(random(1, 10)) * 500, unit: 'ml', toUnit: 'liter', factor: 1 / 1000 },
            { from: floor(random(1, 5)), unit: 'm³', toUnit: 'liter', factor: 1000 },
            { from: floor(random(2, 10)) * 1000, unit: 'liter', toUnit: 'm³', factor: 1 / 1000 },
        ];
        let c = random(conversions);
        let answer = round(c.from * c.factor);
        questions.push({
    ...
    // ===== SOM 10: Massa en gewicht omrekenen =====
    {
        let conversions = [
            { from: floor(random(1, 10)), unit: 'kg', toUnit: 'g', factor: 1000 },
            { from: floor(random(1, 10)) * 500, unit: 'g', toUnit: 'kg', factor: 1 / 1000 },
            { from: floor(random(1, 5)), unit: 'ton', toUnit: 'kg', factor: 1000 },
            { from: floor(random(2, 10)) * 1000, unit: 'kg', toUnit: 'ton', factor: 1 / 1000 },
        ];
        let c = random(conversions);
        let answer = round(c.from * c.factor);
        questions.push({
